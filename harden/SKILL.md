...
description: >-
  ...
  code is written and Review phase is complete. Runs mutation testing to find
...
**前置条件**: 必须先通过 **dev-ownership** 的 Spec→Design→TDD→Review 四个阶段，代码已合并或准备合并。
...
| 步骤 | 谁 | 做什么 |
|------|-----|------|
| **Gate 检查** | AI | 检查 Review 阶段是否已通过 Feynman 门禁。未通过 → 回到 dev-ownership |
...
### Feynman问题
①本轮补了哪些测试？最重要的保护了什么盲区？②循环了几次？第一次补的测试为什么不够？③发现的盲区类型——下一个项目在 TDD 阶段你会先测哪种？
...
- **盲区模式** → 最高频模式写入"热点盲区"，下一轮 TDD 主动建议
