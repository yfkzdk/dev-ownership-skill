...
6. **测试的事全归 harden + verify-spec。** dev-ownership 只负责决策正确性。
...
|| **1: AI 出题** | 3-4 个选项，一个有缺陷。AI 不说哪个。**产出一份跨模型审查材料: 每个选项+缺陷理由+搜索证据。标注: "[CROSS-MODEL] 请独立判断哪个选项有缺陷, 缺陷是什么。"** 开发者将此材料交给其他模型审查 |
|| **2: 你选+识别** | 你选方案 + **指出哪个有缺陷及原因**。未发现→AI 追问 |
|| **3: 反方挑战** | "如果 [具体场景]，方案 A 会怎么失败？" |
|| **4: 揭示+对照** | **跨模型审查结果回来后**: AI 揭示缺陷+证据→对比你的判断 vs 跨模型判断。两者一致→记录 ADR。两者不一致→DISPUTED标记，你裁决 |
...
|| **实现** | D3 完成后 | AI 按选定方案写代码，开发者逐行审查。pre-commit hook 自动卡 lint/type/安全。代码写完后进入 Decision Audit | 代码+commit |
|| **AI自检** | D1/D2/D3都执行?/误导确实不合理?/ADR冲突已记录?/代码逐行审查过? |
...
|| **A5: 代码质量** | AI 报 pre-commit hook 结果(lint/type/安全) + Harden 检出率。开发者对照 ADR 决策→代码实现的一致性 | pre-commit 报告 |
|| **AI自检** | 每个ADR回溯了?/失误根因?/新盲区已写入?/代码质量数据完整? |
