...
def run_mutation_engine(src_dir: Path, root: Path, n_samples: int,
                        python: str) -> Optional[dict]:
    """Run the built-in mutation engine. Returns None if engine unavailable."""
    engine_script = THIS_DIR / "mutation-engine.py"
    ...
    cmd = [
        ...
        "--src", str(src_dir),
        "--test-cmd", f"{python} -m pytest tests/ -q",
        "--n", str(n_samples),
        "--mode", "s",
        "--json",
    ]
    try:
        ...
        return {
            ...
            "score": data["detection_rate"],
            "survivors": [{"location": f'{s["file"]}:{s["lineno"]}',
                           "mutation": s.get("type", "")}
                          for s in data.get("details", {}).get("survived", [])],
            "engine": "builtin",
        }
...
def run_mutation(src_path: str, root: Path, n_samples: int,
                 mutatest: str, python: str, engine_first: bool = True) -> dict:
    """Run mutation test. Tries built-in engine first, falls back to mutatest."""
    import os

    # Try built-in engine first
    if engine_first:
        src_dir = root / "src"
        if not src_dir.exists():
            src_dir = root
        result = run_mutation_engine(src_dir, root, n_samples, python)
        if result and result["total"] > 0:
            return result

    # Fall back to mutatest
    env = os.environ.copy()
...
def main():
    ...
    use_builtin = (THIS_DIR / "mutation-engine.py").exists()
    if not use_builtin and not mutatest:
        print("Error: No mutation engine available (builtin or mutatest)")
        sys.exit(1)
    if not python:
